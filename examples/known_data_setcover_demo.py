...
def load_digits_dataset() -> Tuple[pd.DataFrame, str]:
    ...
    # Select a subset of pixel features and discretize them
    selected_pixels = [f'pixel_{i}_{j}' for i in range(8) for j in range(8)][:20]  # First 20 pixels
...
def create_enhanced_analysis(all_results: Dict[str, Dict[int, Dict[str, Any]]], k_values: List[int]):
    ...
    print("-" * 50)
    print(f"• Small problems (k ≤ 10): Use EXACT for optimal solutions")
    print(f"• Real-time applications: Use GREEDY (fastest, reliable)")
    print(f"• Balanced needs: Use ILP with time limits")
    print(f"• Large problems (k > 20): Use SA or GREEDY")
    print(f"• Quality-critical: Use EXACT or ILP")
