...
try:
    from rowvoi import is_key, minimal_key_advanced
    from rowvoi.setcover import SetCoverAlgorithm  # noqa: F401
except ImportError:
...
def generate_synthetic_data(
    n_rows: int, n_cols: int, sparsity: float = 0.5
) -> pd.DataFrame:
...
def benchmark_algorithm(
    df: pd.DataFrame, rows: list[int], algorithm: str, timeout: float = 60.0, **kwargs
) -> dict[str, Any]:
    ...
    **kwargs
    ...
    try:
        ...
        solution = minimal_key_advanced(
            df, rows, algorithm=algorithm, random_seed=42, **kwargs
        )
        end_time = time.time()
...
def run_benchmark_suite(
    ...
    sparsity_list: list[float] | None = None,
    ...
    Parameters
    ----------
    n_rows_list : List[int], optional
        List of row counts to test
    n_cols_list : List[int], optional
        List of column counts to test
    sparsity_list : List[float], optional
    ...
    if n_rows_list is None:
        n_rows_list = [10, 50, 100]
    if n_cols_list is None:
        n_cols_list = [5, 10, 20]
    if sparsity_list is None:
        sparsity_list = [0.3, 0.5, 0.8]
    if algorithms is None:
        algorithms = ["greedy", "exact", "ilp", "sa", "ga"]
    results = []
    total_configs = len(n_rows_list) * len(n_cols_list) * len(sparsity_list)
    config_num = 0

    print(
        f"Running benchmark suite: {total_configs} configurations × "
        f"{len(algorithms)} algorithms × {n_trials} trials"
    )
    ...
    for n_rows in n_rows_list:
        for n_cols in n_cols_list:
            for sparsity in sparsity_list:
                config_num += 1
                print(
                    f"[{config_num}/{total_configs}] Testing {n_rows} rows × "
                    f"{n_cols} cols, sparsity={sparsity:.1f}"
                )
                ...
                for trial in range(n_trials):
                    ...
                    rows = list(
                        range(min(n_rows, 20))
                    )  # Limit to first 20 rows for tractability
...
def analyze_results(results: list[dict[str, Any]]) -> None:
    ...
    if len(successful) > 0:
        ...
        config_winners = successful.loc[
            successful.groupby("config_id")["runtime"].idxmin()
        ]
        ...
        quality_stats = (
            successful.groupby("algorithm")["solution_size"]
            .agg(["mean", "std"])
            .round(2)
        )
        print(quality_stats)
    ...
    if len(errors) > 0:
        ...
        error_counts = (
            errors.groupby(["algorithm", "error"]).size().unstack(fill_value=0)
        )
        print(error_counts)
        print()
    ...
    if len(successful) > 0:
        ...
        print(
            f"• Most reliable algorithm: {reliable_alg} "
            f"({success_rates.loc[reliable_alg, 'success_rate']:.1f}% success)"
        )
        ...
        print(
            f"• Fastest algorithm on average: {fastest_alg} "
            f"({avg_times[fastest_alg]:.3f}s)"
        )
        ...
        print(
            f"• Best solution quality: {best_quality_alg} "
            f"(avg size={avg_sizes[best_quality_alg]:.1f})"
        )
...
def main():
    """Run main benchmark with command line interface."""
    parser = argparse.ArgumentParser(
        description="Benchmark rowvoi set cover algorithms"
    )
    parser.add_argument(
        "--rows",
        nargs="+",
    ...
    parser.add_argument(
        "--algorithms",
        nargs="+",
        default=["greedy", "exact", "ilp", "sa", "ga"],
        choices=["greedy", "exact", "ilp", "sa", "ga", "hybrid", "lp"],
        help="Algorithms to benchmark",
    )
    parser.add_argument(
        "--timeout", type=float, default=30.0, help="Timeout per algorithm in seconds"
    )
    parser.add_argument(
